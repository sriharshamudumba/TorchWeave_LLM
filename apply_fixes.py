...
def main():
    ...
    if run_command(
        """curl -X POST "http://localhost:8001/models/load" -H "Content-Type: application/json" -d '{"model_id": "distilgpt2"}' """,
        "Loading test model in Model Manager",
    ):
    ...
    if run_command(
        """curl -X POST "http://localhost:8000/models/load" -H "Content-Type: application/json" -d '{"model_id": "distilgpt2"}' """,
        "Loading test model in TorchWeave Server",
    ):
