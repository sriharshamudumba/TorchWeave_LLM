...
class KVCacheManager:
    ...
    def allocate_request_cache(
        self, request_id: str, initial_seq_len: int = 1
    ) -> RequestCache:
        """
    ...
    def get_cache(
        self, request_id: str, layer_idx: Optional[int] = None
        ...
        if layer_idx is not None:
            ...
            return (block.key[:, :, :seq_len, :], block.value[:, :, :seq_len, :])
    ...
    def get_batch_cache(
        self, request_ids: List[str], layer_idx: int
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
