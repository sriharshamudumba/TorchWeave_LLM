...
class TorchWeaveModelLoader:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"[SUCCESS] Model Manager is healthy: {health.get('service', 'unknown')}"
                    )
                    return True
                else:
                    print(
                        f"[ERROR] Model Manager health check failed: {response.status_code}"
                    )
                    return False
        except Exception as e:
            print(
                f"[ERROR] Cannot connect to Model Manager at {self.model_manager_url}: {e}"
            )
            return False

    async def list_loaded_models_safe(
        ...
        for attempt in range(3):
            try:
                async with httpx.AsyncClient(timeout=20.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if not quiet and models:
                            print(
                                f"[INFO] {server_name} - loaded models ({len(models)}):"
                            )
                            for model in models:
                ...
                if not quiet:
                    print(
                        f"[WARNING] Error listing models from {server_name} (attempt {attempt+1}): {e}"
                    )
                if attempt < 2:
    ...
    async def load_model_on_server(self, model_name: str):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=600.0) as client:
                print(
                    f"[DEBUG] POST {self.model_manager_url}{endpoint} with payload: {payload}"
                )
                response = await client.post(
                    f"{self.model_manager_url}{endpoint}", json=payload
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"[ERROR] Load failed on Model Manager: HTTP {response.status_code}: {response.text}"
                    )
                    return False
    ...
    async def wait_for_model_load(self, model_name: str, max_wait_seconds: int = 180):
        ...
        for i in range(max_wait_seconds):
            models = await self.list_loaded_models_safe(
                self.model_manager_url, "Model Manager", quiet=True
            )

            mm_ready = any(
                m.get("model_id", "").lower() == model_name.lower()
                and m.get("status", "").lower() in ["loaded", "available", "unknown"]
                for m in models
            )
        ...
        print(
            f"[WARNING] Model {model_name} did not fully load within {max_wait_seconds} seconds"
        )
        return False
    ...
    async def test_model_generation(self, model_name: str):
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                response = await client.post(
                    f"{self.model_manager_url}/models/generate", json=test_payload
                )
                ...
                if response.status_code == 200:
                    ...
                    if generated_text and generated_text.strip():
                        ...
                        print(
                            f"[WARNING] Model Manager responded but generated empty text"
                        )
                        return False
                else:
                    print(
                        f"[ERROR] Model Manager generation test failed: HTTP {response.status_code}"
                    )
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(
                    f"{self.model_manager_url}/models/available"
                )
                if response.status_code == 200:
                    models_data = response.json()
                    ...
                    print(
                        f"[WARNING] Could not fetch available models: {response.status_code}"
                    )
                    return []
        except Exception as e:
    ...
    async def load_model_comprehensive(self, model_name: str, force_load: bool = False):
        ...
        current_models = await self.list_loaded_models_safe(
            self.model_manager_url, "Model Manager"
        )
        ...
        print(
            f"[WARNING] Wait for load failed, but attempting generation test anyway..."
        )
        return await self.test_model_generation(model_name)
...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://127.0.0.1:8001", help="TorchWeave Model Manager URL"
    )
    parser.add_argument(
        "--list-available", action="store_true", help="List available models and exit"
    )
    parser.add_argument(
        "--list-loaded", action="store_true", help="List loaded models and exit"
    )
    parser.add_argument(
        "--force-load",
        action="store_true",
    ...
    parser.add_argument(
        "--test-only", action="store_true", help="Only test generation without loading"
    )
