...
async def run_sse(client, url, max_new=64):
    ...
    async with client.stream(
        "POST", url, json={"prompt": PROMPT, "max_new_tokens": max_new}
    ) as r:
...
async def bench(url, mode="sse", concurrency=8, iters=50):
    ...
    print(
        f"  tokens ~= {toks}  avg_req_time ~= {avg:.3f}s  agg_tok_per_sec ~= {toks/max(1e-9,total):.2f}"
    )
...
if __name__ == "__main__":
    ...
    if args.sse:
        asyncio.run(
            bench(args.sse, mode="sse", concurrency=args.concurrency, iters=args.iters)
        )
    else:
        asyncio.run(
            bench(
