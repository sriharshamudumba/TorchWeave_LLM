...
class TorchWeaveServerBenchmark:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"[WARNING] Main server unhealthy: {response.status_code}"
                        )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            f"[ERROR] Model manager unhealthy: {response.status_code}"
                        )
                except Exception as e:
    ...
    async def list_loaded_models_on_server(
        self, server_url: str, server_name: str, quiet: bool = False
    ):
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if not quiet:
                        print(
                            f"[WARNING] Failed to list models from {server_name}: {response.status_code}"
                        )
                    return []
        except Exception as e:
            if not quiet:
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                response = await client.post(
                    f"{self.server_url}/models/load", json=payload
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"[INFO] Could not load on main server (HTTP {response.status_code})"
                    )
                    return False
        except Exception as e:
    ...
    async def ensure_model_available(self, model_name: str) -> Dict[str, bool]:
        ...
        # Check Model Manager
        mm_models = await self.list_loaded_models_on_server(
            self.model_manager_url, "Model Manager", quiet=True
        )
        mm_has_model = any(
            m.get("model_id", "").lower() == model_name.lower()
            and m.get("status", "").lower() in ["loaded", "available"]
            for m in mm_models
        )

        # Check Main Server
        ms_models = await self.list_loaded_models_on_server(
            self.server_url, "Main Server", quiet=True
        )
        ms_has_model = any(
            m.get("model_id", "").lower() == model_name.lower()
            and m.get("status", "").lower() in ["loaded", "available"]
        ...
        if mm_has_model and not ms_has_model:
            print(
                f"[INFO] Model available on Model Manager, attempting to load on Main Server..."
            )
        ...
        availability = {"model_manager": mm_has_model, "main_server": ms_has_model}
    ...
    async def _send_baseline_request(
        self,
        model_id: str,
        prompt: str,
        max_tokens: int,
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.post(
                    f"{self.model_manager_url}/models/generate", json=payload
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"[ERROR] Request {request_id} failed with status code: {response.status_code} - {response.text}"
                    )
                    return None
        except Exception as e:
    ...
    async def _send_optimized_request(
        self,
        model_id: str,
        prompt: str,
        max_tokens: int,
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    tokens_generated = metrics.get(
                        "token_count",
                        metrics.get(
                            "estimated_token_count", len(generated_text.split())
                        ),
                    )
                    throughput = tokens_generated / total_time if total_time > 0 else 0
                    ...
                    print(
                        f"[ERROR] Optimized request {request_id} failed on {server_name}: {response.status_code} - {response.text}"
                    )
                    return None
        except Exception as e:
            print(
                f"[ERROR] Optimized request {request_id} failed on {server_name}: {e}"
            )
            return None

    async def run_baseline_benchmark(
        self, model_id: str, prompts: List[str], max_tokens: int = 30
    ):
        ...
        for i, prompt in enumerate(prompts):
            result = await self._send_baseline_request(
                model_id, prompt, max_tokens, 0.7, f"BASE-{i+1}"
            )
            if result:
        ...
        self._print_summary(
            "Baseline (Sequential)", baseline_results, baseline_total_time
        )
        return baseline_results, baseline_total_time

    async def run_optimized_benchmark(
        self, model_id: str, prompts: List[str], max_tokens: int, has_main_server: bool
    ):
        ...
        tasks = []
        for i, prompt in enumerate(prompts):
            task = self._send_optimized_request(
                model_id, prompt, max_tokens, 0.7, f"OPT-{i+1}", has_main_server
            )
            tasks.append(task)
        ...
        optimized_results = [
            r for r in optimized_results if isinstance(r, BenchmarkResult)
        ]
        ...
        if not optimized_results and has_main_server:
            ...
            for i, prompt in enumerate(prompts):
                task = self._send_optimized_request(
                    model_id, prompt, max_tokens, 0.7, f"OPT-{i+1}", False
                )
                tasks.append(task)
        ...
        self._print_summary(
            "Optimized Processing", optimized_results, optimized_total_time
        )
        return optimized_results, optimized_total_time

    def _print_summary(
        self, title: str, results: List[BenchmarkResult], total_time: float
    ):
        ...
        avg_throughput = (
            statistics.mean(r.throughput for r in results) if results else 0
        )
        ...
        print(
            f"Overall Throughput: {total_tokens / total_time:.2f} tokens/sec"
            if total_time > 0
            else "N/A"
        )
        print(f"--------------------------------------------------")
...
3. Custom prompts:
   python3 detailed_benchmark.py --model "microsoft/phi-2" --custom-prompts "Hello" "How are you?" "Tell a joke" --max-tokens 100
    ...
    parser.add_argument(
        "--prompt", default="artificial intelligence", help="Base prompt"
    )
    ...
    if args.model:
        ...
        if not availability["model_manager"]:
            print(
                f"[ERROR] Model '{args.model}' not available on Model Manager. Please load it first."
            )
            return
    else:
    ...
    baseline_results, baseline_time = await benchmark.run_baseline_benchmark(
        model_id, test_prompts, args.max_tokens
    )
    ...
    if baseline_results and optimized_results:
        ...
        baseline_throughput = (
            baseline_total_tokens / baseline_time if baseline_time > 0 else 0
        )
        ...
        optimized_throughput = (
            optimized_total_tokens / optimized_time if optimized_time > 0 else 0
        )
        ...
        latency_change = (
            ((optimized_avg_latency - baseline_avg_latency) / baseline_avg_latency)
            * 100
        ...
        print(f"{'='*80}")
        print(
            f"  Overall Time Improvement: {time_improvement:+.1f}% ({'faster' if time_improvement > 0 else 'slower'})"
        )
        print(f"  Throughput Improvement: {throughput_improvement:+.1f}%")
        print(
            f"  Individual Latency Change: {latency_change:+.1f}% ({'faster' if latency_change < 0 else 'slower'})"
        )
        ...
        if time_improvement > 0:
            success_indicators.append(
                f"✓ {time_improvement:.1f}% faster overall processing"
            )
        if throughput_improvement > 5:
            success_indicators.append(
                f"✓ {throughput_improvement:.1f}% higher throughput"
            )
        if (
            latency_change < 100
        ):  # Reasonable latency increase for concurrent processing
        ...
        if len(test_prompts) == 1:
            ...
            efficiency_factor = (
                baseline_time / optimized_time if optimized_time > 0 else 1
            )
