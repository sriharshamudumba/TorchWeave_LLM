    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        try:
            response = await client.post(
                f"{model_manager_url}/models/generate", json=test_payload
            )
        ...
        for endpoint in alt_endpoints:
            try:
                response = await client.post(
                    f"{server_url}{endpoint}", json=test_payload
                )
            ...
            try:
                response = await client.post(
                    f"{model_manager_url}{endpoint}", json=test_payload
                )
