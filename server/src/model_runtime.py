...
class ModelRuntimeWithKVCache:
        ...
        try:
            ...
            try:
                generation_config = GenerationConfig.from_pretrained(
                    model_id, pad_token_id=tokenizer.eos_token_id
                )
            except:
    ...
    def generate_with_cache(
        ...
        """
        model, tokenizer, gen_config, cache_manager = self._get_model_components(
            model_id
        )
        ...
        try:
            ...
            with torch.no_grad():
                for step in range(max_new_tokens):
                    ...
                    self._update_request_cache(
                        cache_manager, request_id, outputs.past_key_values, step
                    )
            ...
            logger.error(
                f"Generation failed for request {request_id}: {e}", exc_info=True
            )
    ...
    def _get_past_key_values(
        self, cache_manager: KVCacheManager, request_id: str
    ) -> Tuple:
        """
    ...
    def generate_batch_with_cache(
        self, requests: List[Dict[str, Any]], model_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
