...
async def load_model(request: ModelLoadRequest):
    ...
    try:
        ...
        loaded_model_ids = [m["model_id"] for m in current_models.get("loaded_models", [])]
        ...
        updated_loaded = [m["model_id"] for m in updated_models.get("loaded_models", [])]
        
        if request.model_id not in updated_loaded:
            logger.error(f"Model {request.model_id} load reported success but model not found in loaded models")
            raise HTTPException(status_code=500, detail="Model load verification failed")
