...
async def load_model(request: ModelLoadRequest):
    ...
    try:
        ...
        current_models = runtime.list_models()
        loaded_model_ids = [
            m["model_id"] for m in current_models.get("loaded_models", [])
        ]

        if request.model_id in loaded_model_ids:
            logger.info(f"Model {request.model_id} already loaded")
            return {
                "success": True,
        ...
        updated_models = runtime.list_models()
        updated_loaded = [
            m["model_id"] for m in updated_models.get("loaded_models", [])
        ]

        if request.model_id not in updated_loaded:
            logger.error(
                f"Model {request.model_id} load reported success but model not found in loaded models"
            )
            raise HTTPException(
                status_code=500, detail="Model load verification failed"
            )
...
async def generate(request: GenerateRequest):
    ...
    if not runtime or not runtime.models:
        raise HTTPException(
            status_code=400, detail="No models loaded. Please load a model first."
        )
    ...
    try:
        ...
        # Convert request to dict for scheduler
        request_dict = {
        ...
        result = await scheduler.submit_request(**request_dict)
        ...
        queue_time = max(
            0, total_latency - (end_time - start_time)
